...
METROID = 0  # a relic of the past

class Track:
    def __init__(self, volume=0.5, instrument=None, measures=16, timeSig=4, isMuted=False):
        # measures is the base number of measures containing timeSig number of beats
        self.noteList = []
        self.exportNoteList = []
        self.exportNoteNameList = []
...
global drop_down_options
drop_down_options = list(fourierCoefficients.keys())
...
def mixTracks(trackList, song_name):
    ...
    trackWavs = []
    for track in trackList:
        if (track.isMuted == False) and (track.volume != 0):
            track.exportNoteList = track.noteList[0:lastMeasureIndex + 1]
            track.exportNoteNameList = track.noteNameList[0:lastMeasureIndex + 1]
            print(track.noteList)
            print(track.exportNoteNameList)
            print(track.noteDurationList)
            sound1 = track.exportNoteList[0][0] + ((track.volume - 0.5) * 30)
            for measureNum in range(len(track.exportNoteList)):
                if len(track.exportNoteList[measureNum]) == 1 and measureNum == 0:  # if the first measure contains one note
                    sound2 = 0  # 0 so that the only sound in measure 1 isn't doubled in amplitude
                    newSound = sound1 + sound2
                    sound1 = newSound
                elif len(track.exportNoteList[measureNum]) == 1:  # if non-first measure is only one note long
                    sound2 = track.exportNoteList[measureNum][0] + ((track.volume - 0.5) * 30)
                    newSound = sound1 + sound2
                    sound1 = newSound
                elif measureNum == 0:  # cover for rest of 1st measure cases
                    for note in range(1, len(track.exportNoteList[measureNum])):
                        sound2 = track.exportNoteList[measureNum][note] + ((track.volume - 0.5) * 30)
                        newSound = sound1 + sound2
                        sound1 = newSound
                else:
                    for note in range(0, len(track.exportNoteList[measureNum])):
                        sound2 = track.exportNoteList[measureNum][note] + ((track.volume - 0.5) * 30)
                        newSound = sound1 + sound2
                        sound1 = newSound
            s = 'TrackTestNum' + str(trackList.index(track)+1) + '.wav'
...
class MainWin(Canvas):
    ...
    def setUP(self):
        global track_list
        #vol buttons
        t1vol = Button(self.master, text = "T1 vol", fg = "sky blue", bg = "black", width = int(WIDTH/2), height = int(HEIGHT/6), command = lambda:self.TrackVolGUICall("Track 1 Volume", track_list[0]))
        t1vol.place(bordermode = OUTSIDE, width = int((WIDTH/18)+(1/3)), height = int(HEIGHT/6))
        t2vol = Button(self.master, text = "T2 vol", fg = "sky blue", bg = "black", width = int(WIDTH/2), height = int(HEIGHT/6), command = lambda:self.TrackVolGUICall("Track 2 Volume", track_list[1]))
        t2vol.place(bordermode=OUTSIDE, width = int((WIDTH/18)+(1/3)), height = int(HEIGHT/6), y = 100)
        t3vol = Button(self.master, text = "T3 vol", fg = "sky blue", bg = "black", width = int(WIDTH/2), height = int(HEIGHT/6), command = lambda:self.TrackVolGUICall("Track 3 Volume", track_list[2]))
        t3vol.place(bordermode=OUTSIDE, width = int((WIDTH/18)+(1/3)), height = int(HEIGHT/6), y = 200)
        t4vol = Button(self.master, text = "T4 vol", fg = "sky blue", bg = "black", width = int(WIDTH/2), height = int(HEIGHT/6), command = lambda:self.TrackVolGUICall("Track 4 Volume", track_list[3]))
        t4vol.place(bordermode=OUTSIDE, width = int((WIDTH/18)+(1/3)), height = int(HEIGHT/6), y = 300)
        t5vol = Button(self.master, text = "T5 vol", fg = "sky blue", bg = "black", width = int(WIDTH/2), height = int(HEIGHT/6), command = lambda:self.TrackVolGUICall("Track 5 Volume", track_list[4]))
        t5vol.place(bordermode=OUTSIDE, width = int((WIDTH/18)+(1/3)), height = int(HEIGHT/6), y = 400)
        t6vol = Button(self.master, text = "T6 vol", fg = "sky blue", bg = "black", width = int(WIDTH/2), height = int(HEIGHT/6), command = lambda:self.TrackVolGUICall("Track 6 Volume", track_list[5]))
    ...
    def helpCall(self):
        ...
        tkinter.messagebox.showinfo("Synth Documentation", "Reading the GUI:\n\
        - t1m1 (Track 1, Measure 1)\n\n\
        Adding Notes:\n\
        - Clicking on a Measure button will pull up a window that allows you to add notes\n\
        - Enter the name of the note in the 'Note Name' entry box (Example: C4 / Db5)\n\
        - Enter the letter 'R' for rests\n\
        - Beats can be in decimal form\n\
        - All accidentals are written in flats\n\
        - Pressing the 'Enter' key to automatically add the note\n\n\
        Copying Measures:\n\
        - The notes that are copied will retain the instrument of the original measure copied\n\n\
        Exporting songs:\n\
        - The program will retain all notes when exported\n\
        - The program will rewrite any wav file with the same name as the current project\n")
    ...
    def MeasureGUICall(self, name, track, mes):
        ...
        add_note_button = Button(top, text = "Add Note", font=("MingLiU-ExtB", 12), bg="black", fg="light blue", command = lambda:self.noteAdd(track, mes, N_N.get(), Num_B.get(), volu.get(), B_start.get(), instru.get()))
        add_note_button.place(width=int(mWIDTH / 2), height=int(mHEIGHT/12), y=int(mHEIGHT/12) * 5)
        top.bind("<Return>", lambda event:self.noteAdd(track, mes, N_N.get(), Num_B.get(), volu.get(), B_start.get(), instru.get()))
        ...
        instrument_dropdown = OptionMenu(top, instru, *drop_down_options)
        instrument_dropdown.place(width=int(mWIDTH / 2), height=int(mHEIGHT/12), y=int(mHEIGHT/12) * 6, x=int(mWIDTH / 2))
        instrument_dropdown.config(bg="black", fg="light blue", font = ("MingLiU-ExtB", 12))
        instrument_dropdown["menu"].config(bg="gray", fg="blue", font=("MingLiU-ExtB", 12))
    ...
    def TrackVolGUICall(self, track_title,track):
        top = Toplevel(window, bg = "black")
        top.geometry("{}x{}".format(mWIDTH, mHEIGHT))
        top.title(track_title)
        Label(top, text=track_title, font=("MingLiU-ExtB", 12), bg = "black", fg = "light blue").place(width=mWIDTH, height=int(mHEIGHT/6))
        global track_list
        global NewTrackVolumeLabel
        for i in track_list:
            if i == track:
                TrackVolumeLabel = Label(top, text='Current Volume: {}'.format(track.volume), font=("MingLiU-ExtB", 12), bg = "black", fg = "light blue")
                TrackVolumeLabel.place(x=mWIDTH/2.5, y=50)
        VolumeSlider = Scale(top, from_=0, to_=10, orient=HORIZONTAL,font=("MingLiU-ExtB", 12), bg = "black", fg = "light blue")
        VolumeSlider.set(5)
        NewTrackVolumeLabel = Label(top, text='New Volume:'.format(VolumeSlider.get()), font=("MingLiU-ExtB", 12), bg = "black", fg = "light blue")
        NewTrackVolumeLabel.place(x=mWIDTH/2.3, y=140)
        if track.isMuted == False:
            MuteLabel = Label(top, text='Unmuted', font=("MingLiU-ExtB", 12), bg = "black", fg = "light blue")
            MuteLabel.place(x=mWIDTH/2.3, y=120)
        else:
            MuteLabel = Label(top, text='Muted', font=("MingLiU-ExtB", 12), bg = "black", fg = "red")
            MuteLabel.place(x=mWIDTH/2.3, y=120)
        Button(top, text='Set Volume',command=lambda:self.ButtonUpdate(TrackVolumeLabel,VolumeSlider.get(),track),font=("MingLiU-ExtB", 12), bg = "black", fg = "light blue").place(x=mWIDTH/2.1, y=90)
        Button(top, text='Mute',command=lambda:self.MuteUpdate(MuteLabel,track),font=("MingLiU-ExtB", 12), bg = "black", fg = "light blue").place(x=mWIDTH/2.6, y=90)
        #the volume slider place has to be down here because I guess tkinter no like otherwise
        VolumeSlider.place(x=mWIDTH/2.3, y=160)
        
    def ButtonUpdate(self, label, Volume, track):
        track.volume = round(Volume/10,1)
        label.config(text='Current Volume: {}'.format(round(Volume/10,1)))
        
    def MuteUpdate(self, label, track):
        if label['text'] == 'Unmuted':
            track.isMuted = True
            label.config(text='Muted',fg = "red")
        else:
            track.isMuted = False
            label.config(text='Unmuted',fg = "light blue")
            
    def noteAdd(self, track, measure, note_name, note_length, note_vol, note_start, instrument):
        trackDict[str(track)].addNote(measure-1, int(note_start)-1, note_name.capitalize(), instrument, float(note_vol), float(note_length))
